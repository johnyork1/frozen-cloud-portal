...
def load_catalog():
    ...
    if catalog_path.exists():
        with open(catalog_path, 'r') as f:
            ...
            return [s for s in full_catalog.get("songs", []) if s.get("act_id") == "FROZEN_CLOUD"]
    return []

# Load songs (filtered to Frozen Cloud only)
songs = load_catalog()
...
if page == "Dashboard":
    ...
    if songs:
        ...
        for s in songs:
            table_data.append({
                "Title": s['title'],
                "Artist": s.get('artist', 'Frozen Cloud'),
...
# ============================================================================
    ...
    st.caption("View where Frozen Cloud songs are distributed and streaming")
